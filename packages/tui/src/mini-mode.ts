...
export const miniMode = (store: GlobalStore) => {
  store.subscribe(
    ...
    (status, prev) => {
      ...
      if (status == "aggregators:initializing") {
        output += eventTitle("Initializing aggregator...");
        output = chalk.dim(output);
        output = chalk.hex("#F5E663")(MARKER_CHAR) + output;
      }

      if (status === "strategies:initializing") {
        output += eventTitle("Initializing strategies...");
        output = chalk.dim(output);
        output = chalk.hex("#F5E663")(MARKER_CHAR) + output;
      }

      if (status === "wallets:loading") {
        output += eventTitle("Loading wallets...");
        output = chalk.dim(output);
        output = chalk.hex("#F5E663")(MARKER_CHAR) + output;
      }

      if (status === "wallets:validating") {
        output += eventTitle("Validating wallets...");
        output = chalk.dim(output);
        output = chalk.hex("#F5E663")(MARKER_CHAR) + output;
      }

      if (status === "aggregator:computingRoutes") {
        const computedRoutesPerMinute =
          state.chart.computedRoutesPerSecond.values.reduce(
            (acc, curr) => acc + curr,
            0
          );

        const nonZeroLatencyValues =
          state.chart.computeRoutesLatency.values.filter((v) => v > 0);
        const latencyAvg =
          nonZeroLatencyValues.reduce((acc, curr) => acc + curr, 0) /
          nonZeroLatencyValues.length;
      ...
      if (status === "aggregator:computingRoutesSuccess") {
        ...
        const expectedProfitPercent =
          currentStrategy.expectedProfitPercent.value;
      ...
      if (status === "aggregator:execute:executing") {
        ...
        output += `Expected Profit: ${expectedProfitPercent.value.toFixed(
          8
        )} % | `;
        output += `${inAmount?.toFixed(8)} ${inTokenSymbol} >>> `;
        output += `${outAmount?.toFixed(8)} ${outTokenSymbol}`;
      ...
      if (status === "history:successfulTx") {
        ...
        const unrealizedProfitPercent =
          latestSuccessfulTx?.unrealizedProfitPercent;
        const profitPercent =
          latestSuccessfulTx?.profitPercent?.toFixed(8) ?? 0;
